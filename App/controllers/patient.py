...
def create_patient(firstname, lastname, username, password, email, phone_number):
    if len(username) > 120:
        ...
        return None
    if len(firstname) > 120:
        print("Firstname too long")  
        return None
    if len(lastname) > 120:
        print("Lastname too long")  
        return None
    if len(email) > 120:
...
def create_medical_history(patient_id, age, dob, gender, blood_type, weight, height, allergies, medical_conditions, medication):
